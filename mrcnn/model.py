...
# Requires TensorFlow 2.6+ and Keras 2.6.0+.
from distutils.version import LooseVersion
assert LooseVersion(tf.__version__) >= LooseVersion("2.6")
...
class PyramidROIAlign(KL.Layer):
    ...
    def call(self, inputs):
        ...
        box_range = tf.expand_dims(
            tf.range(tf.shape(input=box_to_level)[0]), 1)
...
def overlaps_graph(boxes1, boxes2):
    ...
    overlaps = tf.reshape(
        iou, [tf.shape(input=boxes1)[0], tf.shape(input=boxes2)[0]])
    return overlaps
